...
// Fetches variant data from Shopify's storefront endpoint — no template injection needed.
(function() {
  function fmt(cents) {
    return '$' + (cents / 100).toFixed(2).replace(/\.00$/, '');
  }

  // Extract product handle from URL e.g. /products/my-ring
  ...
  fetch('/products/' + handle + '.js')
    ...
    .then(function(product) {
      ...
      console.log('MN: loaded', variants.length, 'variants for', handle);
    ...
    .catch(function(e) { console.error('MN: fetch failed', e); });
})();
